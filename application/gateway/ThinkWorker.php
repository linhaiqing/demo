<?php
/**
 * 继承 Workerman Worker 重写 parseCommand 方法
 * 令它能适合在 Conosle 模式下接收参数
 * User: admin
 * Date: 2018/10/11
 * Time: 10:09
 */
...
class ThinkWorker extends Worker
{
    ...
    protected static function parseCommand() {
        ...
        if (!isset(self::$args[1]) || !in_array(self::$args[1], $available_commands)) {
            if (isset(self::$args[1])) {
                static::safeEcho('Unknown command: ' . self::$args[1] . "\n");
            }
            exit($usage);
        }

        // Get command.
        $command  = trim(self::$args[1]);
        $command2 = isset(self::$args[2]) ? self::$args[2] : '';
